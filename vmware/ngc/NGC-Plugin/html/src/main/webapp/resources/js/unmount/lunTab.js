...
function initData() {
    ...
    var url = ns.webContextPath + "/rest/data/host/unmountableVolumeList/" + hostId + "?serverGuid=" + serverGuid +
    ...
    var lunhandler = new handler(function doSuccess(resp) {
        if (resp.errorCode) {
            parent.$("#diverrorLUN").text(resp.errorDesc).show();
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            $lunRow.children("td").each(function (num, td) {
                ...
                if (isEmpObj($(td).attr("name"))) {
                    ...
                    if (arr[i]["usedType"] == "Datastore") {
                        url = "../../../assets/images/Datastore.png";
                        td.innerHTML = "<img src=" + url + ">" + "&nbsp;&nbsp;" + arr[i]["usedBy"];
                        td.title = arr[i]["usedBy"];
                    } else if (arr[i]["usedType"] == "VirtualMachine") {
                        url = "../../../assets/images/VirtualMachine.png";
                        td.innerHTML = "<img src=" + url + ">" + "&nbsp;&nbsp;" + arr[i]["usedBy"];
                        td.title = arr[i]["usedBy"];
                    } else {
                        td.innerHTML = arr[i]["usedBy"];
                        td.title = arr[i]["usedBy"];
                    }
                }
...
function loacalSetPageCheckBox(rowNum) {
    ...
    parent.$("#chk_all").click(function () {
        ...
        if (chkAll) {
            if (rowNum > 0) {
                parent.$("#btnUnmount").prop("disabled", "");
                parent.$("#btnUnmount .plugin_button_div").css("cursor", "pointer");
            }
        } else {
            parent.$("#btnUnmount").prop("disabled", "disabled");
            parent.$("#btnUnmount .plugin_button_div").css("cursor", "default");
        }
        $("input[id^='diveChbox_']").each(function (i) {
            if (chkAll) {
                if (!this.checked) {
