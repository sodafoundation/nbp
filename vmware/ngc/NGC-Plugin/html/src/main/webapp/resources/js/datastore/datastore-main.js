...
var objectId = ""; //dataCenter/Lun的id
...
$(document).ready(function() {
	...
	$("#preStep").click(function() {
		...
		if(currentPage == 6) {
			if(datastoreType == "lunDatastore" && (isCreateDatastore==true)) {
				$("#mainFrame").attr("src", "block/block-p4-datastoreName.html");
				$("#nextStep").val("CREATE");
			} else if (datastoreType == "lunDatastore" && (isCreateDatastore==false)) {
                $("#mainFrame").attr("src", "block/block-p3-lunInfo.html");
                $("#nextStep").val("CREATE");
            } else if(datastoreType == "nfsDatastore"){
...
function createDatastore() {
	lunCapa = getCapaValue(lunCapa + lunCapaUnit);
	initCapa = getCapaValue(initCapa + initCapaUnit);
	useableCapa = getCapaValue(useableCapa);
	...
	if(datastoreType == "lunDatastore") {
		createDatastoreInfo.fileVersion = fileVersion;
    ...
    if (type=="createDatastoreOrLun"){
        url = "/rest/datastore/createDatastores?objectId=" + hostIdArray + "&serverGuid=" + serverGuid;
    } else {
